...
class UserInput():
    ...
    def get_help(self):
        ...
        description.append(('pdb_trajectory_filename', "The filename of the multi-frame PDB to analyze. Individual frames should be separated by \"END\" or \"ENDMDL\" lines."))
        description.append(('output_directory', "A new directory where the WISP output should be written. If this parameter is not specified, a default output directory is created whose name includes the current date for future reference."))

        # Covariance-matrix related
        description.append(('title', "COVARIANCE-MATRIX PARAMETERS"))
        description.append(('node_definition', "WISP calculates the covariance matrix by defining nodes associated with each protein residue. If node_definition is set to \"CA,\" the alpha carbon will be used. If set to \"RESIDUE_COM,\", \"SIDECHAIN_COM,\", or \"BACKBONE_COM,\" the whole-residue, side-chain, or backbone center of mass will be used, respectively."))
        description.append(('contact_map_distance_limit', 'If you use WISP\'s default contact-map generator, node pairs with average inter-node distances greater than this value will not be considered in calculating the covariance matrix.'))
        description.append(('load_wisp_saved_matrix', 'If the covariance matrix (appropriately modifed by a contact map) has been previously saved to a file, set this parameter to "TRUE" to load the matrix instead of generating it from scratch. WISP automatically saves a copy of this matrix to the file "functionalized_matrix_with_contact_map_applied.pickle" in the output directory every time it is run.'))
        description.append(('wisp_saved_matrix_filename', 'If load_wisp_saved_matrix is set to "TRUE," this parameter specifies the file to load. If it is set to "FALSE," this parameter specifies the file to which the matrix should be saved.'))

        # Path related
        description.append(('title', "PATH-SEARCHING PARAMETERS"))
        description.append(('desired_number_of_paths', 'One of the advantages of WISP is that it can calculate not only the optimal path between residues, but multiple good paths. This parameter specifies the desired number of paths.'))
        description.append(('source_residues', 'This parameter specifies the source residues for path generation. A list of residues should be constructed of the form "CHAIN_RESNAME_RESID," separated by spaces. For example: "X_SER_1 X_LEU_4." For unix to treat a space-containing command-line parameter as a single parameter, it must be enclosed in quotes.'))
        description.append(('sink_residues', 'This parameter specifies the sink residues for path generation. The format is the same as for the source_residues parameter.'))

        # Multi-processor related
        description.append(('title', "MULTI-PROCESSOR PARAMETERS"))
        description.append(('number_processors', 'On unix-like machines, WISP can use multiple processors to significantly increase speed. This parameter specifies the number of processors to use.'))
        description.append(('num_frames_to_load_before_processing', 'When WISP is run with multiple processors, the frames from the PDB are loaded in chunks before being distributed to the many processors. This parameter specifies the number of frames to load before distribution.'))

        # Visualization
        description.append(('title', "VISUALIZATION PARAMETERS"))
        description.append(('shortest_path_radius', 'WISP outputs a VMD state file to facilitate visualization. The shortest path is represented by a strand with the largest radius. Longer paths have progressively smaller radii. This parameter specifies the radius of the shortest path, in Angstroms.'))
        description.append(('longest_path_radius', 'This parameter specifies the radius of the longest path visualized, in Angstroms.'))
        description.append(('spline_smoothness', 'The paths are represented by splines connecting the nodes. This parameter indicates the smoothness of the splines. Smaller values produce smoother splies, but take longer to render.'))
        description.append(('vmd_resolution', 'When visualizing in VMD, a number of cylinders and spheres are drawn. This parameter specifies the resolution to use.'))
        description.append(('node_sphere_radius', 'When visualizing in VMD, spheres are placed at the locations of the nodes. This parameter specifies the radius of these spheres.'))
        description.append(('shortest_path_r', 'The color of the shortest path is given by an RGB color code. This parameter specifies the R value, ranging from 0.0 to 1.0.'))
        description.append(('shortest_path_g', 'The color of the shortest path is given by an RGB color code. This parameter specifies the G value, ranging from 0.0 to 1.0.'))
        description.append(('shortest_path_b', 'The color of the shortest path is given by an RGB color code. This parameter specifies the B value, ranging from 0.0 to 1.0.'))
        description.append(('longest_path_r', 'The color of the longest path is given by an RGB color code. This parameter specifies the R value, ranging from 0.0 to 1.0.'))
        description.append(('longest_path_g', 'The color of the longest path is given by an RGB color code. This parameter specifies the G value, ranging from 0.0 to 1.0.'))
        description.append(('longest_path_b', 'The color of the longest path is given by an RGB color code. This parameter specifies the B value, ranging from 0.0 to 1.0.'))
        description.append(('node_sphere_r', 'The color of the node spheres is given by an RGB color code. This parameter specifies the R value, ranging from 0.0 to 1.0.'))
        description.append(('node_sphere_g', 'The color of the node spheres is given by an RGB color code. This parameter specifies the G value, ranging from 0.0 to 1.0.'))
        description.append(('node_sphere_b', 'The color of the node spheres is given by an RGB color code. This parameter specifies the B value, ranging from 0.0 to 1.0.'))
        description.append(('shortest_path_opacity', 'The opacity of the shortest path, ranging from 0.0 (transparent) to 1.0 (fully opaque). Note that if --shortest_path_opacity, --longest_path_opacity, and --node_sphere_opacity are not all identical, the output TCL file will contain many materials, which may be less-than-desirable for some users.'))
        description.append(('longest_path_opacity', 'The opacity of the longest path, ranging from 0.0 (transparent) to 1.0 (fully opaque).'))
        description.append(('node_sphere_opacity', 'The opacity of the node spheres, ranging from 0.0 (transparent) to 1.0 (fully opaque).'))
        description.append(('pdb_single_frame_filename', 'By default, WISP uses the trajectory-average structure for positioning the nodes, visualizing the paths and protein, etc. However, if desired, a separate PDB structure with the same residue order and number can be specified for this purpose using the "pdb_single_frame_filename" parameter.'))

        # Advanced features
        description.append(('title', "ADVANCED FEATURES"))
        description.append(('seconds_to_wait_before_parallelizing_path_finding', 'WISP identifies paths from the source to the sink by recursively visiting node neighbors. The program begins the recursion algorithm on a single processor before distributing the search efforts to multiple processors. This parameter specifies how long WISP should search for source-sink paths using a single processor before distributing the search effort over multiple processors. By waiting longer before distribution, the search efforts are ultimately distributed more evenly over the multiple processors, potentially increasing speed in the long run. On the other hand, specifiying a lower value for this parameter means the program will spend more time running on multiple processors, also potentially increasing speed. A balance must be struck.'))
        description.append(('user_specified_functionalized_matrix_filename', 'A text file containing a user-specified functionalized correlation matrix. If not given, WISP\'s default functionalized correlation matrix, as described in the WISP publication, will be automatically calculated. For convenience, WISP automatically saves a human-readable copy of the matrix used to the file "functionalized_correlation_matrix.txt" in the output directory every time it is run.'))
        description.append(('user_specified_contact_map_filename', 'A text file containing a user-specified contact map. If given, each element of the functionalized matrix will be multiplied by the corresponding value specified in the file. If not given, WISP\'s default contact map, based on the distances between average node locations, will be automatically applied. For convenience, WISP automatically saves a human-readable copy of the contact-map matrix to the file "contact_map_matrix.txt" in the output directory every time it is run.'))
...
class GetCovarianceMatrix():
    ...
    def __init__(self, params):
        ...
        for residue_iden in self.average_pdb.residue_identifiers_in_order:
            dot_products = (set_of_deltas[residue_iden] * set_of_deltas[residue_iden]).sum(axis=1)
        ...
        if params['user_specified_functionalized_matrix_filename'] == '':
            ...
            for x in range(len(self.average_pdb.residue_identifiers_in_order)):
                ...
                for y in range(len(self.average_pdb.residue_identifiers_in_order)):
                    ...
                    C = ensemble_average_dot_products / numpy.power(ensmeble_average_deltas_self_dotproducted[residue1_key] * ensmeble_average_deltas_self_dotproducted[residue2_key], 0.5)
        ...
        if params['user_specified_contact_map_filename'] == '':
            if params['contact_map_distance_limit'] != 999999.999:
                ...
                for index1 in range(len(self.average_pdb.residue_identifiers_in_order)-1):
                    ...
                    for index2 in range(index1+1,len(self.average_pdb.residue_identifiers_in_order)):
    ...
    def convert_list_of_residue_keys_to_residue_indices(self, list_residue_keys):
        ...
        for key in list_residue_keys:
            index_of_key = numpy.nonzero(self.average_pdb.residue_identifiers_in_order==key)[0][0]
...
class Visualize():
    ...
    def __init__(self, params, corr_matrix_object, pths):
        ...
        log("mol new " + molecule_filename + " type pdb first 0 last -1 step 1 filebonds 1 autobonds 1 waitfor all", log_files)
...
def output_directory_info(params):
    ...
    f.write("If you prefer not to use the command line, simply run the vmd executable and load the \"visualize.vmd\" file using \"File->Load Visualization State...\" from the main menu." + "\n\n")
    f.write("The above methods are very slow. If your output is so large that a faster option is required, the Tk Console can be used. Use \"Extensions->Tk Console\" from the VMD main menu to pull up the Tk Console. Then run the following command, with the full path to \"visualize.tcl\" included if necessary:" + "\n\n")
    f.write("source visualize.tcl" + "\n\n")
    f.write("Regardless of the method you use to load in the WISP output, the visualization will be the same. Individual pathways are shown as tubes (i.e., \"wisps\"), the protein is shown in ribbon representation, and protein residues that participate in any path are shown in licorice representation." + "\n\n")
    f.write("The WISP output directory contains a number of other files as well. Here are descriptions of each:" + "\n\n")
    f.write("log.txt: Details describing WISP execution." + "\n\n")
    f.write("parameters_used.txt: The WISP parameters used to generate the output." + "\n\n")
    f.write("average_structure.pdb: The average structure of your PDB trajectory." + "\n\n")
    f.write("draw_frame.pdb: If the user requests that a separate single-structure PDB file be used for calculating node and wisp positions, that file is saved as \"draw_frame.pdb\". Otherwise, the average structure is used." + "\n\n")
    f.write("functionalized_matrix_with_contact_map_applied.pickle: A python pickle file that contains the matrix obtained by multiplying a functionalized correlation matrix and a contact map. This file is not human readable but can be loaded into WISP for use in subsequent runs with the -load_wisp_saved_matrix and -wisp_saved_matrix_filename parameters. Thus, the matrix needs only to be calculated once for each trajectory, rather than every time WISP is executed. Use \"python wisp.py -help\" for more information." + "\n\n")
    f.write("contact_map_matrix.txt: A human readable representation of the contact map. If the user wishes to generate their own contact map rather than letting WISP generate one automatically, a custom contact map formatted like this one can be loaded into WISP using the -user_specified_contact_map_filename parameter. " + "\n\n")
    f.write("functionalized_correlation_matrix.txt: A human readable representation of the functionalized correlation matrix, prior to multiplication by the contact map. If the user wishes to generate their own functionalized correlation matrix rather than letting WISP generate one automatically, a custom matrix formatted like this one can be loaded into WISP using the -user_specified_functionalized_matrix_filename parameter. " + "\n\n")
    f.write("simply_formatted_paths.txt: A simple list of path lengths and nodes. The first column contains the lengths, and all following columns contain node indices. This file may be helpful for subsequent statistical analyses of the WISP output." + "\n")
    f.close()
...
if __name__=="__main__":
    ...
    sinks = correlation_matrix_object.convert_list_of_residue_keys_to_residue_indices(parameters['sink_residues'])
    ...
    log("\n# Program execution time: " + str(time.time() - program_start_time) + " seconds", parameters['logfile'])
